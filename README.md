...
独立的 AI × 长寿研究信号雷达。每小时完整接入通用 AI 雷达的公开来源，并叠加论文数据库、临床试验注册库、研究机构 RSS、Papers.cool 和 GitHub 项目。网站默认展示“相关精选”，核心交集优先、生命延续背景研究补充；全部信息源保留为高级审计层。
...
- `data/latest-relevant.json`：近 7 天核心交集 + 近 21 天相关研究，最多 60 条。
...
网站默认打开“相关精选”：近 7 天核心交集优先，并从近 21 天生命延续背景研究或高价值 AI 生物研究工具中补充；内容不足时不使用通用 AI 填数量。用户可切换到“核心交集”或“全部信息源”。只有同时达到两类阈值的条目才属于核心交集并进入 `latest-24h.json`。输出还会标注相关层级、入选路径、研究对象、发表阶段、证据类型和风险提示。这里的“高分”表示主题信号较强，不表示治疗有效、适用于人类或具备临床价值。
